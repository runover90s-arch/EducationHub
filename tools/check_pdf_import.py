...
for ex in sorted(GRADE.glob('0[1-4]-*/practice/*/exercises.md')):
    ...
    if '## Ngân hàng bài tập PDF mở rộng' not in et:
        continue
    files+=1
    if not sol.exists():
    ...
    if '## Đáp án và lời giải — Ngân hàng PDF mở rộng' not in st:
        errors.append(f'Thiếu phần lời giải PDF: {sol.relative_to(ROOT)}')
        continue
    es=split_imported(et,'## Ngân hàng bài tập PDF mở rộng')
    ss=split_imported(st,'## Đáp án và lời giải — Ngân hàng PDF mở rộng')

    em=list(re.finditer(r'^#### Bài PDF (\d+)\s*\n\n<!-- source-id: ([^>]+) -->\n\n',es,re.M))
    sm=list(re.finditer(r'^#### Bài PDF (\d+)\s*$',ss,re.M))
    en=[int(m.group(1)) for m in em]; sn=[int(m.group(1)) for m in sm]
    if en != list(range(1,len(en)+1)):
        errors.append(f'Số Bài PDF không liên tục: {ex.relative_to(ROOT)}')
    if sn != en:
    ...
    for i,m in enumerate(em):
        ...
        if active:
            ...
            level=fmt='Không rõ'
            warnings.append(f'Không xác định nhóm mức độ: {ex.relative_to(ROOT)} Bài PDF {seq}')

        # exact duplicate textual question (image-only questions use source image and are keyed by source-id instead)
        nb=norm_text(block)
        if len(nb)>150 and 'công thức/kí hiệu của câu này được giữ nguyên bằng ảnh' not in nb:
            key=hashlib.sha1(nb.encode()).hexdigest()
            if key in body_seen:
                errors.append(f'Câu PDF trùng nguyên văn: {ex.relative_to(ROOT)} Bài {seq} và {body_seen[key]}')
            else:
                body_seen[key]=f'{ex.relative_to(ROOT)} Bài {seq}'

        # image references must resolve relative to file
        for ref in re.findall(r'!\[[^]]*\]\(([^)]+\.webp)\)',block):
            target=(ex.parent/ref).resolve()
            if not target.exists():
                errors.append(f'Ảnh bài tập không tồn tại: {ex.relative_to(ROOT)} -> {ref}')
        if 'source-faithful/' in block:
            image_q+=1
        ...
        if 'source-faithful/' in sb:
            solution_img+=1

        # Vận dụng cao must have an actual explanation or a preserved solution image.
        if level.startswith('Vận dụng cao'):
            plain=re.sub(r'\*\*Đáp án:\*\*[^\n]*','',sb)
            plain=norm_text(plain)
            if len(plain)<80 and 'source-faithful/' not in sb:
                errors.append(f'Vận dụng cao thiếu lời giải chi tiết: {sol.relative_to(ROOT)} Bài PDF {seq}')
...
print(f'[pdf-bank] {image_q} câu dùng ảnh giữ nguyên công thức/kí hiệu; {solution_img} lời giải dùng ảnh.')
